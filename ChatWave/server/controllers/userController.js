...
module.exports.register = async (req, res, next) => {
  try {
    ...
    // Kiểm tra nếu mật khẩu không có trong yêu cầu
    if (!password) {
      return res
        .status(400)
        .json({ msg: "Password is required", status: false });
    }
    ...
    if (emailCheck) {
      return res.status(200).json({ msg: "Email da ton tai", status: false });
    }
    ...
    // Xoá mật khẩu trước khi trả về thông tin người dùng
...
module.exports.sendFriendRequest = async (req, res, next) => {
  try {
    ...
    if (friend.friendRequests.includes(userId)) {
      return res
        .status(400)
        .json({ msg: "Friend request already exists from this user" });
    }
